...
func TestChartMigration(t *testing.T) {
	...
	// Create legacy chartconfig CR.
	{
		...
		_, err := config.K8sClients.G8sClient().CoreV1alpha1().ChartConfigs(key.Namespace()).Create(ctx, chartConfig, metav1.CreateOptions{})
		if err != nil {
			t.Fatalf("expected %#v got %#v", nil, err)
		}
	...
	// Create chart CR.
	{
		...
		_, err := config.K8sClients.G8sClient().ApplicationV1alpha1().Charts(key.Namespace()).Create(ctx, chart, metav1.CreateOptions{})
		if err != nil {
			t.Fatalf("expected %#v got %#v", nil, err)
		}
	...
	// Add annotation to chartconfig CR to mark that migration is complete.
	{
		...
		chartConfig, err := config.K8sClients.G8sClient().CoreV1alpha1().ChartConfigs(key.Namespace()).Get(ctx, key.TestAppReleaseName(), metav1.GetOptions{})
		if err != nil {
			t.Fatalf("expected %#v got %#v", nil, err)
		}
		...
		_, err = config.K8sClients.G8sClient().CoreV1alpha1().ChartConfigs(key.Namespace()).Update(ctx, chartConfig, metav1.UpdateOptions{})
		if err != nil {
			t.Fatalf("expected %#v got %#v", nil, err)
		}
	...
	// Delete chartconfig CR.
	{
		...
		err := config.K8sClients.G8sClient().CoreV1alpha1().ChartConfigs(key.Namespace()).Delete(ctx, key.TestAppReleaseName(), metav1.DeleteOptions{})
		if err != nil {
			t.Fatalf("expected %#v got %#v", nil, err)
		}
	...
	// Check chartconfig CR is deleted.
	{
		...
		o := func() error {
			_, err := config.K8sClients.G8sClient().CoreV1alpha1().ChartConfigs(key.Namespace()).Get(ctx, key.TestAppReleaseName(), metav1.GetOptions{})
