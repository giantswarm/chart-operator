...
func (c *Collector) collectTillerConfigured(ctx context.Context, ch chan<- prometheus.Metric) {
	...
	if err != nil {
		c.logger.LogCtx(ctx, "level", "error", "message", fmt.Sprintf("could not get Charts"), "stack", fmt.Sprintf("%#v", err))
		return
	}
	...
	if len(charts) == 0 && len(chartConfigs) == 0 {
		...
		c.logger.Log("level", "error", "message", "did not collect Tiller configuration")
...
func (c *Collector) checkTillerDeployment() error {
	deploy, err := c.k8sClient.Extensions().Deployments(c.tillerNamespace).Get("tiller-deploy", metav1.GetOptions{})
	...
	for _, envVar := range containers[0].Env {
		if envVar.Name == "TILLER_HISTORY_MAX" && envVar.Value == "10" {
			// Configuration is correct.
			return nil
		}
	}

	return microerror.Maskf(invalidExecutionError, "tiller configuration is incorrect")
}
